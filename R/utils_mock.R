...
#'  - simulate per-sample beta values with random CpG-level noise,
...
create_mock_hvCpG_data <- function(
  ...
  # Generate per-dataset means & variances to make them distinct
  for (i in seq_len(n_datasets)) {
    sample_idx <- which(datasets == paste0("DS", i))
    # dataset-level mean methylation profile
    dataset_mean <- runif(n_cpgs, 0.3, 0.7)
    # per-sample noise around that mean
    for (j in sample_idx) {
      mat[, j] <- pmin(pmax(rnorm(n_cpgs, mean = dataset_mean, sd = runif(1, 0.05, 0.15)), 0), 1)
    }
  }
  ...
  medsd_lambdas <- lapply(unique(datasets), function(ds) {
    ...
    if (ncol(submat) >= 3) {
      row_sds <- apply(submat, 1, sd, na.rm = TRUE) ## sd per CpG
      median_sd <- median(row_sds, na.rm = TRUE) ## median sd for a dataset
      p95 <- quantile(row_sds, 0.95, na.rm = TRUE) ## 95 quantile
  ...
  # Transpose so that samples = rows, CpGs = columns
  mat_t <- t(mat)

  rhdf5::h5write(mat_t, h5_path, "matrix")
